...
(function() {
  ...
  window.onload = function() {
    ...
    if (document.body.classList.contains('qotd')) {
      keeperConversation();
      setTimeout(function(){
        window.alert("Press on the yellow text to input." +
                      " Unfortunately the ajax calls become cached" +
                    " and do not update as expected... you'll see." +
                    " Will fix soon!" +
                    " Also, was forced to break the module-global pattern "
                   + "to implement JASONP for the random quote API."
                   + " \n APIs used: Lorem Picsum, Quotes on Design");
      }, 1000);
    }
    ...
    if (document.body.classList.contains('home')) {  // if on home, execute quote
      ...
      quoteInfo();
      setTimeout(function(){
        window.alert("Change background image with arrow keys, press author name for more info."+
        " You can also log your stay on the bottom left corner." +
        " Try visiting the creative diary section too." +
        " WARNING: If viewing on atom, prompt popups do not work for some reason." +
        " A browser will do!");
      }, 1500);
    }
  };
